...
import { AllModules, GridApi, GridOptions, GridSizeChangedEvent, CellKeyDownEvent, SuppressKeyboardEventParams, PostSortRowsParams, TabToNextCellParams, CellPosition } from '@ag-grid-enterprise/all-modules';
...
@Component({
  selector: 's4-grid',
  templateUrl: './alpine-grid.component.html',
  styleUrls: ['./alpine-grid.component.scss']
})
export class AlpineGrid {
  @Input() id: string;
  ...
  public defaultPaginationPageSize = this.uiCustomizeService.gridConfig && this.uiCustomizeService.gridConfig.pagType? +this.uiCustomizeService.gridConfig.pagType : 10;
  ...
  @Input() public checkValidationOnAdd = true;
  ...
  @Input() public showAllButton : boolean = true; 
  ...
  @Input() rightClicked: (any) => ContextMenuData | undefined;
  ...
  constructor(public translateService: TranslateService,
    ...
    this.defaultColDef = {
      ...
      suppressKeyboardEvent: (params) => {
        return this.suppressSpaceKey(params);
      },
      floatingFilter: (this.uiCustomizeService && this.uiCustomizeService.gridConfig && this.uiCustomizeService.gridConfig.filterType == '2')
    };
  ...
  addEditableData(col, agcol, canEdit: boolean) {
      ...
      case 'number':
        {
          ...
          if (col.rightAlign) {
            agcol['rightAlign'] = col.rightAlign;
          }
          break;
        }
      case 'phone':
        {
          ...
          if (col.formatType) {
            agcol['formatType'] = col.formatType;
          }
          break;
        }
      case 'text':
    ...
    if (col.hyperLinkText && col.displayas === 'href' && col.datatype === 'hyperlink'){
    ...
    if (col.datatype === 'launchbutton'){
       agcol['cellClass'] = 'ag-cell-launchbutton';
       agcol['minWidth'] = 200
    }
    if (col.styleClass === 'cell-flow-wrap'){
      agcol['cellClass'] = 'cell-flow-wrap';
    }
    if (col.datatype === 'number' && col.rightAlign === true){
      // agcol['headerClass'] = 'ag-align-left';
      agcol['cellClass'] = 'align-right';
    }
    /*if (filterparams) {
  ...
  setGridWidth() {
    ...
    if(this.gridColumnApi){
      let allDisplayedColumns = this.gridColumnApi.getAllDisplayedColumns();
      let columnWidth = 0;
      let dispalyedColumnsId = [];
      for (let i =0; i < allDisplayedColumns.length; i++  ) {
         dispalyedColumnsId.push(allDisplayedColumns[i].colId)
         columnWidth+= allDisplayedColumns[i].actualWidth;
      }
      this.gridColumnApi.autoSizeColumns(dispalyedColumnsId, false);
    }
  }
  ...
  checkRequiredFieldValidation(rowDataArr) {
    ...
    for (const row of rowDataArr) {
      for (const requiredField of requiredFields) {
        let cellData = row[requiredField.field];
        cellData = typeof cellData == 'string' ? cellData.trim() : cellData;
        if (cellData === undefined || cellData === '' || cellData === null) {
          this.type = 'warn';
          this.message = requiredField.headerName + ' ' + this.translateService.translate('common.mustbeentered');
          this.show();
          return false;
        }
      }
  ...
  validationForSavedRecord(){
    const added = [];
    this.addedMap.forEach((value) => { added.push(value); });
    const updated = [];
    const mapSort = new Map([...this.updatedMap.entries()].sort());
    mapSort.forEach((value) => { updated.push(value); });
    
    if(updated.length > 0 && !this.checkRequiredFieldValidation(updated)){
      return false;
    }
    if(added.length > 0 && !this.checkRequiredFieldValidation(added)){
      return false;
    }
    return true;
  }
  ...
  addRecord(gridOptions) {
    if(this.checkValidationOnAdd && !this.checkRequiredFieldValidation(this.gridOptions.rowData)){
      return;
    }
      ...
      if (selectedData && selectedData.length > 0) {
        ...
        if (res.remove) {
          for (const node of res.remove) {
            ...
            this.rowData.splice(nextIndex, 1); 
          }
        }
        if (nextIndex >= 0) {
  ...
  onRowClicked(){
   // this.eoffenderService.selectedRowData=null;
    ...
    if (this.rowsSelected && this.rowSelection === 'single') {
      this.rowsSelected = this.rowsSelected[0];
     /*  if(this.rowsSelected.createDate){
        this.eoffenderService.selectedRowData=this.rowsSelected;
      } */
     /*  const coldefs = this.columnDefs.filter(element => {
        return element.field === 'isContexual';
      });
      if (coldefs.length > 0) {
        this.eoffenderService.selectedRowData=this.rowsSelected;
      } */
  ...
  clearRecords(gridOptions) {
    ...
    this.clearedData.emit({ cleared: this.addedMap, currentData: this.rowData});
    this.refreshGridSize();
  }
  ...
  onSave(gridOptions) {
    ...
    // wait till editing stop
    let myInterval = setInterval(() => {
      if (!this.gridEditMode) {
        clearInterval(myInterval);
        this.proceedToSave(gridOptions);
      }
    }, 100);

  }

  proceedToSave(gridOptions) {
    if(!this.validationForSavedRecord()){
      return;
    }
    this.gridOptions.api.clearFocusedCell();
    this.clearFilter();
  ...
  onCellValueChanged(event) {
    if(event.colDef.coltype == 'time' || event.colDef.coltype == 'date'){
      if((event.oldValue == null && event.newValue == undefined) || (new Date(event.oldValue).getTime() == new Date(event.newValue).getTime())){
        return;
      }
    }
    let isSavable = true;
    ...
    this.updatedMapsData.emit({ updated: event.data, field: event.colDef.field });
  }
  ...
  onCellKeyDown(e: CellKeyDownEvent) {
    if (this.gridEditMode) {
      ...
      let inputEle = (e.event.target as HTMLInputElement);
      let latestValue = inputEle.type == "number" ? inputEle.valueAsNumber : inputEle.value;
      if(latestValue && startingValue !== latestValue && !(e.colDef["coltype"] == 'lov')) {
        this.btnSavebtnDisable = false;
        this.btnClearbtnDisable = false;
      }
    }
      ...
      if (this.gridOptions && this.gridOptions.api && this.columnDefs) {
        ...
        this.columnDefs.forEach(column => {
          if (column.datatype && column.datatype === 'select' || column.datatype === 'lov') {
             this.gridOptions.api.destroyFilter(column.field);
          }
        });
          this.searchValue = '';
          this.gridOptions.api.setQuickFilter(this.searchValue);
  ...
  onPaginationChanged(event) {
    ...
    if (this.gridOptions.api && this.gridOptions.rowData && this.gridOptions.rowData.length > 0) {
      let isFilterApplied = this.gridApi.getFilterModel();
      if(Object.keys(isFilterApplied).length){
        this.handleSelectedRowAfterFilter();
      }
  ...
  updatePaginationSize() {
    if(this.paginationPageSize && typeof this.paginationPageSize == 'number'){
  ...
  getContextMenuItems(params) {
    let columnField = params.column.colId;
    let rowData = params.node.data;
    let rowID =  params.node.id;
    let source = params.column.colDef.source;
    let sourceDomain = params.column.colDef.sourceDomain;

    let data;
    if (this.rightClicked && typeof this.rightClicked === 'function') {
      data = this.rightClicked({rowData : rowData, columnField: columnField, rowID: rowID});
    }
    ...
    let isAccessable = false;
	  ...
	  if(this.sessionManager.userRoles.roles[source] === 'full' || this.sessionManager.userRoles.roles[source] === 'read') {
	        isSourceAccessable = true;
	  }
    const router = this.router;
    const dialog = this.dialog;
    // IF LOV HAD ONLY DOMAIN
    ...
    // IF LOV HAD SOURCE &  SOURCEdOMAIN BOTH
    else if(isSourceAccessable && params.column.colDef.filter === 'lovfilter' &&  source && sourceDomain) {
      	var result1 = [
	        {
	          name: 'Source : ' + source,
	          disabled: true,
	        ...
	        },
          {
	        ...
	        },
	        {
	          ...
	          action: function () {
	            dialog.dialog.closeAll();
              if(source && params.column.colDef.sourceDomain) {
                router.navigate([source], { queryParams: { domain: params.column.colDef.sourceDomain } });
              } else if(source && !params.column.colDef.sourceDomain) {
	            	router.navigate([source]);
	            }
	          },
	        },'separator','copy','copyWithHeaders','paste','separator',
        ...
        },
        {
          ...
          action: function () {
            ...
            if(source && sourceDomain) {
              router.navigate([source], { queryParams: { domain: sourceDomain } });
            } else if(source && !sourceDomain) {
              router.navigate([source]);
            }
          },
        },'separator','copy','copyWithHeaders','paste','separator',
  ...
  handleSelectedRowAfterFilter() {
    ...
    if (node) {
      ...
      this.btnDeletedDisabled = true;
      if (this.blankNodeID === null) {
        this.blankNodeID = undefined;
        return;
      }
      else if (this.blankNodeID === undefined) {
        this.addTemporaryNode();
      }
      else if (this.blankNodeID) {
  ...
  addTemporaryNode(){
    let newRowData = {blankNode : true};
    if (this.onInsert && typeof this.onInsert === 'function') {
      newRowData = this.onInsert();
      newRowData.blankNode = true;
      if (!newRowData) {
        newRowData = {blankNode : true};
      }
    }
    this.gridOptions.api.applyTransaction({ add: [newRowData] });
    this.gridApi.forEachNode((rowNode, index) => {
      if ('blankNode' in rowNode.data) {
        this.blankNodeID = rowNode.id;
        rowNode.setSelected(true);
        this.onRowClicked()
      }
    });
