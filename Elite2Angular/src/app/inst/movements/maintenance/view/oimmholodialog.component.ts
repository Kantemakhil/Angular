...
@Component({
    selector: 'app-oimmholodialog',
    templateUrl: './oimmholodialog.component.html'
})

export class OimmholoDialogComponent implements OnInit {
    ...
    ngOnInit() {
        this.agyLocId = this.dialog.data.level1Code;
        this.agyLocIdTemp = this.dialog.data.agyLocId;
        this.lvlOneactiveFlag=this.dialog.data.activeFlag 
        this.alertInsert = true;
        this.pageNumber = 'Page 2 of 4';
        ...
        this.livUnitsColumnDef = [
            ...
            },
            {
                ...
                datatype: 'checkbox', editable: this.lvlOneactiveFlag
            },
            {
    ...
    */
    oimmholoSavelivunitsForm(event) {
        ...
        this.livunitsCommitModel.housingLevel = 2;
    ...
    */
    ...
    getIEPCodeExecuteQuery() {
        if(this.livunitsModel.livingUnitId!=undefined){
        ...
        if(this.dialog.data.livingUnitId){
            this.oimmholoFactory.getIEPCodeExecuteQuery(this.dialog.data.livingUnitId,this.agyLocIdTemp).subscribe(parentIepData => {
                if (parentIepData !== AppConstants.EMPTYDATA) {
                    this.parentIepLevel = parentIepData.split(",")[0];
                }
            }) 
        }
    ...
    nextButtonClick = (data) => {
        if ((this.grid.addedMap && this.grid.addedMap.size > 0) ||
            ...
            this.dialogService.openLinkDialog('/OIMMHOLODIALOGONE', this.livunitsModel, 90).subscribe(res => {
                if (res === 'BACKBUTTON') {
                    this.dialog.close(null);
                }
            });
