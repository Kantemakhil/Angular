...
export class OimmholoDialogTwoComponent implements OnInit {
    ...
    ngOnInit() {
        this.agyLocId = this.dialog.data.level3Code;
    ...
    validateRowData = (event) => {
        ...
        if (event.field === 'activeFlag') {
            if (event.data.numbers > 0 && !event.data.activeFlag) {
    ...
    */
    oimmholoSavelivunitsForm(event) {
        ...
        this.livunitsCommitModel.housingLevel = 4;
    ...
    */
    ...
    nextButtonClick = (data) => {
        if ((this.grid.addedMap && this.grid.addedMap.size > 0) ||
            ...
            this.isLoading = true;
            this.livunitsModel['levelOneActiveFlag']=this.dialog.data.levelOneActiveFlag; 
    ...
    getIEPCodeExecuteQuery() {
        if (this.livunitsModel.parentLivingUnitId) {
