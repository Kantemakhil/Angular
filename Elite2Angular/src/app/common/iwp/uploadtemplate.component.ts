...
//                          this.getPropertiesFromFile(file, file.name);
  ...
  extensionImage( filename ) {
    ...
    onClear() {
        console.log(this.fileupRowData);
    }
    ...
    upload() {
        ...
        for (var index = 0; index < this.fileupRowData.length; index++) {
            ...
            if (this.fileupRowData[index].documentType && !this.fileupRowData[index].templateUri && this.templateList) {
            ...
            // this.fileupRowData[index].edrmsRecordNo = this.dialog.data.edrmsRecordNo;
            // this.fileupRowData[index].edrmsFolder = this.dialog.data.edrmsFolder;
            // this.fileupRowData[index].objectType = this.dialog.data.objectType;
        }
        if (this.selectedFilesNew.length == 0) {
            this.type = 'warn';
            ...
            this.oumdtempFactory.pushTemplateFileToStorage(this.selectedFilesNew, this.fileupRowData, this.eOffenderDetails, this.vHeaderBlockModel, this.objectType,'Y').subscribe(resultCount => {
                ...
                if (resultCount === this.fileupRowData.length) {
                    ...
                    this.loaderService.hideLoader();
                    this.type = 'error';
                    this.message = this.translateService.translate('oumdtemp.unsupportedFile');
                    this.show();
                    return;
                }else {
                    this.loaderService.hideLoader();
